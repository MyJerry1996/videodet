...
def parseXmlFilse(image_path, anno_path, save_path, json_name='train.json'):
    ...
    images_index = dict((v.split(os.sep)[-1][:-5], k) for k, v in enumerate(images))
    for file in files:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ap', '--anno-path', type=str, default='./ours/labels', help='yolo txt path')
    parser.add_argument('-s', '--save-path', type=str, default='./ours/annotations', help='json save path')
    parser.add_argument('--image-path', default='./ours/images')
    parser.add_argument('--json-name', default='train.json')

    opt = parser.parse_args()
    parseXmlFilse(opt.image_path, opt.anno_path, opt.save_path, opt.json_name)
    # if len(sys.argv) > 1:
    #     print(opt)
